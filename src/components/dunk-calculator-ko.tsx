...
const formSchema = z.object({
  height: z.coerce.number({ required_error: '키는 필수입니다' }).positive('양수여야 합니다'),
  standingReach: optionalPositiveNumber,
  verticalLeap: z.coerce.number({ required_error: '수직 점프는 필수입니다' }).positive('양수여야 합니다'),
  rimHeight: z.coerce.number().positive('양수여야 합니다'),
});
...
export default function DunkCalculatorKo() {
  ...
  const onSubmit = (values: FormValues) => {
    ...
    const displayUnit = unit === 'metric' ? 'cm' : '인치';
        ...
        <CardContent className="pt-6">
          <Tabs value={unit} onValueChange={(v) => setUnit(v as UnitSystem)} className="w-full mb-6">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="imperial">인치법 (인치)</TabsTrigger>
              <TabsTrigger value="metric">미터법 (cm)</TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="number" placeholder="예: 183" {...field} value={field.value ?? ''} />
                    </FormControl>
                    <FormDescription>{unit === 'imperial' ? '인치' : 'cm'}</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>서 있는 상태에서의 도달 거리 (선택 사항)</FormLabel>
                    <FormControl>
                      <Input type="number" placeholder="추정하려면 비워 두십시오" {...field} value={field.value ?? ''} />
                    </FormControl>
                    <FormDescription>{unit === 'imperial' ? '인치' : 'cm'}</FormDescription>
                    <FormMessage />
                  </FormItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>수직 점프</FormLabel>
                    <FormControl>
                      <Input type="number" placeholder={unit === 'imperial' ? '예: 24' : '예: 61'} {...field} value={field.value ?? ''} />
                    </FormControl>
                    <FormDescription>{unit === 'imperial' ? '인치' : 'cm'}</FormDescription>
                    <FormMessage />
                  </FormItem>
            ...
            <div className="mt-8">
              {result.canDunk ? (
                <div className="relative text-center p-6 bg-accent/20 rounded-lg border border-accent">
                  ...
                  <h3 className="text-2xl font-bold font-headline text-accent-foreground">축하합니다!</h3>
                  <p className="text-accent-foreground/90 mt-2">
                    최대 도달 거리가 <span className="font-bold">{result.maxReach.toFixed(1)} {result.unit}</span>이므로 덩크를 할 수 있습니다! 코트로 나갈 시간입니다.
                  </p>
                </div>
              ) : (
                <div className="text-center p-6 bg-secondary rounded-lg">
                  <Target className="h-16 w-16 text-primary mx-auto mb-4" />
                  <h3 className="text-2xl font-bold font-headline">거의 다 왔습니다!</h3>
                  <p className="text-foreground/80 mt-2 mb-4">
                    덩크를 하려면 수직 점프에 <span className="font-bold text-primary">{result.needed.toFixed(1)} {result.unit}</span>이 더 필요합니다.
                  </p>
                  <div className="space-y-2">
                     <div className="flex justify-between text-sm font-medium text-muted-foreground">
                        <span>현재 최대 도달 거리: {result.maxReach.toFixed(1)} {result.unit}</span>
                        <span>덩크 목표: {result.targetReach.toFixed(1)} {result.unit}</span>
                     </div>
